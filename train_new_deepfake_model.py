#!/usr/bin/env python3
"""
New Deepfake Detection Model Training Script using PyTorch
Trains a CNN model to detect AI-generated vs Real images
Uses New_datasets folder with optimized settings
"""
...
class NewDeepfakeDataset(Dataset):
    """Custom dataset for deepfake detection using New_datasets folder"""
    ...
    def __init__(self, data_dir, transform=None, split='train', val_split=0.2):
        ...
        if os.path.exists(ai_dir):
            ...
            ai_images += glob.glob(os.path.join(ai_dir, '*.jpeg')) + glob.glob(os.path.join(ai_dir, '*.bmp'))
            ai_images += glob.glob(os.path.join(ai_dir, '*.tiff')) + glob.glob(os.path.join(ai_dir, '*.webp'))
        ...
        if os.path.exists(real_dir):
            ...
            real_images += glob.glob(os.path.join(real_dir, '*.jpeg')) + glob.glob(os.path.join(real_dir, '*.bmp'))
            real_images += glob.glob(os.path.join(real_dir, '*.tiff')) + glob.glob(os.path.join(real_dir, '*.webp'))
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Only print error once per corrupted image to avoid spam
            if not hasattr(self, '_corrupted_images'):
                self._corrupted_images = set()
            if img_path not in self._corrupted_images:
                print(f"⚠️  Corrupted image detected: {img_path} - {type(e).__name__}")
                self._corrupted_images.add(img_path)
...
class NewDeepfakeTrainer:
    """Main trainer class for new deepfake detection model"""
    
    def __init__(self, model, device, save_dir='new_training_output'):
...
def check_dataset_health(dataset_path):
    ...
    for folder in ['AI', 'Real']:
        ...
        print(f"📁 Checking {folder} folder: {len(image_files)} images")
        ...
        for img_path in tqdm(image_files, desc=f"Checking {folder}"):
            ...
            try:
                ...
                print(f"❌ Corrupted: {os.path.basename(img_path)} - {type(e).__name__}")
    ...
    if corrupted_files:
        print(f"⚠️  Corrupted images will be replaced with black images during training")
        print("💡 Consider removing or replacing these files for better training:")
        for img_path, error in corrupted_files[:5]:  # Show first 5
            print(f"   - {img_path}")
...
def main():
    parser = argparse.ArgumentParser(description='Train new deepfake detection model with PyTorch')
    parser.add_argument('--dataset_path', default='New_datasets', 
                       help='Path to New_datasets folder')
    parser.add_argument('--model_type', choices=['custom', 'resnet', 'efficientnet'], 
                       default='resnet', help='Type of model to use')
    parser.add_argument('--epochs', type=int, default=50, help='Number of training epochs')
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size (optimized for CPU)')
    parser.add_argument('--img_size', type=int, default=224, help='Image size (square)')
    parser.add_argument('--learning_rate', type=float, default=0.0001, help='Learning rate')
    parser.add_argument('--patience', type=int, default=10, help='Early stopping patience')
    parser.add_argument('--output_dir', default='new_training_output', help='Output directory')
    ...
    if not os.path.exists(args.dataset_path):
        ...
        print("Please make sure your New_datasets folder exists with AI and Real subfolders.")
        return
    
    print("=== New PyTorch Deepfake Detection Training ===")
    ...
    corrupted_count = check_dataset_health(args.dataset_path)
    if corrupted_count > 0:
        response = input(f"\n⚠️  Found {corrupted_count} corrupted images. Continue training? (y/n): ")
        if response.lower() != 'y':
    ...
    # Create datasets
    train_dataset = NewDeepfakeDataset(args.dataset_path, transform=train_transform, split='train')
    val_dataset = NewDeepfakeDataset(args.dataset_path, transform=val_transform, split='val')
    ...
    # Create trainer
    trainer = NewDeepfakeTrainer(model, device, args.output_dir)
